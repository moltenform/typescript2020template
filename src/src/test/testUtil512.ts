...
/* auto */ import { LockableArr, Util512, ValHolder, arLast, assertEq, cast, castVerifyIsNum, castVerifyIsStr, checkThrowEq, findEnumToStr, findStrToEnum, fitIntoInclusive, getEnumToStr, getEnumToStrOrFallback, getShapeRecurse, getStrToEnum, listEnumVals, listEnumValsIncludingAlternates, longstr, slength, sortConsistentType } from './../util/util512';
...
t.test('listEnumVals, listEnumValsIncludingAlternates', () => {
    assertEq(
        '__isUI512Enum, __UI512EnumCapitalize, First, Second, Third, TheFirst, Scnd, Thd',
        listEnumValsIncludingAlternates(TestEnum)
    );
    assertEq(
        '__isUI512Enum, EOne, ETwo, EThree',
        listEnumValsIncludingAlternates(TestSimpleEnum)
    );
...
t.test('getEnumToStrOrFallback, standard usage', () => {
    ...
    // flags like the initial marker should not be visible
    assertEq('fallback', getEnumToStrOrFallback(TestEnum, 1, 'fallback'));
});
...
t.test('sortConsistentType.StringWithNonAscii', () => {
    ...
    assertEq(
        'accentedéletter,accentedéletter,accentedéletter,aunicode♦char,aunicode♧char',
        arr.join(',')
    );
});
...
t.test('sortConsistentType.DiffTypesInArrayShouldThrow', () => {
    ...
    assertThrows('not compare', () =>
        sortedConsistentType([
            ['a', 1],
            ['a', 'a']
        ])
    );
...
t.test('sortConsistentType.DiffTypesInObjectShouldThrow', () => {
    const arr = sortedConsistentType([{ a: 1 }, { a: 2 }, { a: 0 }]);
...
t.test('sortConsistentType.maps and stability', () => {
    ...
    assertEq(
        'a,1,a,2,b,2,b,1',
        sortedConsistentType(arrArrs, (x: any) => x[0])
            .map(String)
            .join(',')
    );
    assertEq(
        'a,1,b,1,b,2,a,2',
        sortedConsistentType(arrArrs, (x: any) => x[1])
            .map(String)
            .join(',')
    );
    const arr = [
    ...
    assertEq(
        ...
        sortedConsistentType(arr)
            .map((x: any) => JSON.stringify(x))
            .join(',')
    );
    assertEq(
        '{"a":1,"b":3},{"a":2,"b":3},{"a":3,"b":1}',
        sortedConsistentType(arr, (x: any) => x.a)
            .map((x: any) => JSON.stringify(x))
            .join(',')
    );
    assertEq(
        '{"a":3,"b":1},{"a":2,"b":3},{"a":1,"b":3}',
        sortedConsistentType(arr, (x: any) => x.b)
            .map((x: any) => JSON.stringify(x))
            .join(',')
    );
});
