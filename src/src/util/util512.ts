...
export function findStrToEnum<T>(Enm: T, s: string): O<T> {
    const EnmUntyped = Enm as Record<any, any>
    assertTrue(
        EnmUntyped['__isUI512Enum']  !== undefined,
    ...
    if (s.startsWith('__')) {
        ...
        if (EnmUntyped['__UI512EnumCapitalize'] !== undefined) {
            s = Util512.capitalizeFirst(s);
        }

        let found = EnmUntyped[s];
        if (found) {
            return found;
        } else {
            return EnmUntyped['__AlternateForm__' + s];
        }
    }
}
...
export function getStrToEnum<T>(Enm: T, msgContext: string, s: string): T {
    ...
    if (found !== undefined) {
        ...
        if (EnmUntyped['__isUI512Enum'] !== undefined) {
            let makeLowercase = EnmUntyped['__UI512EnumCapitalize'] !== undefined;
...
export function findEnumToStr(Enm: TypeLikeAnEnum, n: number): O<string> {
    ...
    for (let enumMember in EnmUntyped) {
        if (
            (EnmUntyped[enumMember] as unknown) === n &&
            !enumMember.startsWith('__') &&
            !enumMember.startsWith('__AlternateForm__')
        ) {
            let makeLowercase = EnmUntyped['__UI512EnumCapitalize'] !== undefined;
